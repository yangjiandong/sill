Ext.namespace('Ext.app.base');

Ext.app.base.Lov = Ext.extend(Ext.form.TriggerField, {
  defaultAutoCreate : {
    tag : "input",
    type : "text",
    size : "16",
  ...
  fieldMapping : []
  // example:
  // fieldMapping:[{column:'ID',field:'DC0014_ID'},{column:'CODE',field:'DC0014_CODE'}]

  ,
  paramMapping : []
  // example:
  // paramMapping:[{param:'p_client_id',field:'DC0014_CLIENT_ID'},{param:'p_bpartner_id',field:'DC0014_BPARTNER_FROM'}]
  ,
  queryArraySize : -1,
  window : null
  ...
  initComponent : function() {
    ...
    this.queryFieldsMap = [new Ext.form.ComboBox({
              ...
              hiddenName : this.dataComponentName + '_QRY_FIELD_ID',
...
/* getQueryFieldLabels() */}), new Ext.form.TextField({
    ...
    if ((this.view.xtype != 'grid' && this.view.xtype != 'dataview')
      ...
      throw "Ext.app.base.Lov.view option must be instance of Ext.grid.GridPanel or Ext.DataView!";
    }
    Ext.app.base.Lov.superclass.initComponent.call(this);
  ...
  ,
  initEvents : function() {
    Ext.app.base.Lov.superclass.initEvents.call(this);
  ...
  onRender : function(ct, position) {
    ...
    Ext.app.base.Lov.superclass.onRender.call(this, ct, position);
  ...
  validateValue : function(value) {
    if (Ext.app.base.Lov.superclass.validateValue.call(this, value)) {
  ...
  executeQuery : function() {
    ...
    // newParamVal = (!Ext.isEmpty(this.paramMapping[i].field))?
    // this.callFromGrid.store.getAt(this.callFromGridRow).get(
    // this.paramMapping[i].field ):this.paramMapping[i].value;
    // } else {
      ...
      if (this.paramMapping[i].field) {
        if (this.paramMapping[i].field.indexOf(".") < 0) {
          if (!Ext.isEmpty(this.callFromGrid)) {
            newParamVal = this.callFromGrid.store.getAt(this.callFromGridRow)
                .get(this.paramMapping[i].field);
          } else {
            newParamVal = Ext.getCmp(this.paramMapping[i].field).getValue();
          }
        } else {
          var dc = this.paramMapping[i].field.substring(0,
              this.paramMapping[i].field.indexOf("."));
          var fld = this.paramMapping[i].field
              .substring(this.paramMapping[i].field.indexOf(".") + 1);
    ...
    // p['_p_query_column_code'] =
    // Ext.getCmp(this.dataComponentName+'_QRY_FIELD_ID').getValue();
  ...
  onDoSelect : function() {
    ...
    Ext.form.TriggerField.superclass.setValue.call(this, this
            .getSelectedRecords()[0].get(this.displayField));

    for (var i = 0; i < this.fieldMapping.length; i++) {
      if (!Ext.isEmpty(this.callFromGrid)) {
        this.callFromGrid.store.getAt(this.callFromGridRow).set(
            this.fieldMapping[i].field,
            this.getSelectedRecords()[0].get(this.fieldMapping[i].column));
      } else {
        Ext.getCmp(this.fieldMapping[i].field).setValue(this
            .getSelectedRecords()[0].get(this.fieldMapping[i].column));
      }
    }
      ...
      this.windowConfig = Ext.apply(this.windowConfig, {
            ...
            tbar : new Ext.Panel({
                  ...
                  bbar : [{
                        ...
                        xtype : "button",
                        cls : "x-btn-text-icon",
                        icon : "resource/icon/lov_select.gif"
                      }, {
                        text : 'Cancel',
                        ...
                        xtype : "button",
                        cls : "x-btn-text-icon",
                        icon : "resource/icon/lov_cancel.gif"
                      }
                  // ,'->'
                  // ,{text:'Refresh', handler:this.onDoRefresh, scope:this,
                  // xtype:"button", cls:"x-btn-text-icon",
                  // icon:"_static/icon/lov_refresh.gif"}
                  ],
                  frame : false,
                  layout : 'table',
                  layoutConfig : {
                  ...
                  tbar : [{
                        xtype : 'label',
                        text : 'Filter on:'
                      }, this.queryFieldsMap[0], this.queryFieldsMap[1], {
                        xtype : 'button',
                        text : 'Ok',
...
Ext.reg('xlovfield', Ext.app.base.Lov);