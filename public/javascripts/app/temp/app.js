  ...
  initComponent: function() {
    ...
    selectClass: function(cls) {
                   if (cls) {
                     ...
                     var res = [];
                     var pkg = [];
                     for (var i = 0; i < last; i++) { // things get nasty - static classes
                       // can have .
                       var p = parts[i];
                       var fc = p.charAt(0);
...
Ext.extend(MainPanel, Ext.TabPanel, {
  ...
  initSearch: function() {
                ...
                var resultTpl = new Ext.XTemplate('<tpl for=".">', '<div class="search-item">', '<a class="member" ext:cls="{cls}" ext:member="{member}" href="output/{cls}.html">', '<img src="javascripts/ext/resources/images/default/s.gif" class="item-icon icon-{type}"/>{member}', '</a> ', '<a class="cls" ext:cls="{cls}" href="output/{cls}.html">{cls}</a>', '<p>{doc}</p>', '</div></tpl>');
...
Ext.onReady(function() {
  ...
  var statusBar = new Ext.BoxComponent({
    region: 'south',
      height: 16,
      autoEl: {
        html: '<div id="footer"><p>Powered by jror &copy;  '+ dss.copyright + '  - All rights reserved. 版权所有</p></div>'
      }
  });
...
Ext.extend(Ext.ux.SelectBox, Ext.form.ComboBox, {
  ...
  initEvents: function() {
    ...
    // you need to use keypress to capture upper/lower case and shift+key,
